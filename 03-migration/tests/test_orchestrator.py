...
from orchestrator import get_total_unmigrated_products, execute_cloud_run_job, run_orchestrator
...
def test_get_total_unmigrated_products():
    ...
    with patch("orchestrator.init_db_engine", return_value=mock_engine), \
         patch("orchestrator.sessionmaker", return_value=mock_sessionmaker):
...
@patch("orchestrator.get_total_unmigrated_products", return_value=0)
