...
def test_write_products_success():
    ...
    with patch("src.db_writer.init_db_engine", return_value=mock_engine), \
         patch("src.db_writer.sessionmaker", return_value=mock_sessionmaker):
        ...
        assert mock_session.execute.called or mock_session.add_all.called or mock_session.add.called
...
def test_write_products_exception():
    ...
    with patch("src.db_writer.init_db_engine"), \
         patch("src.db_writer.sessionmaker", return_value=mock_sessionmaker):
...
def test_init_db_engine():
    with patch("src.db_writer.Connector"), \
         patch("src.db_writer.sqlalchemy.create_engine") as mock_create_engine:
