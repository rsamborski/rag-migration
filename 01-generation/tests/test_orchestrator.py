...
def test_run_orchestrator_success():
    ...
    with patch("orchestrator.get_total_products", return_value=250), \
         patch("orchestrator.execute_cloud_run_job", return_value=mock_operation) as mock_execute, \
         patch("orchestrator.poll_execution") as mock_poll:
...
def test_run_orchestrator_zero_rows():
    with patch("orchestrator.get_total_products", return_value=0), \
         patch("orchestrator.execute_cloud_run_job") as mock_execute:
...
def test_poll_execution_success():
    ...
    with patch("orchestrator.run_v2.ExecutionsClient", return_value=mock_client), \
         patch("orchestrator.time.sleep"): # Skip sleep
