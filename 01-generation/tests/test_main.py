...
def test_run_worker_success():
    ...
    with patch.dict(os.environ, env), patch(
        "main.fetch_products", return_value=mock_products
    ) as mock_fetch, patch(
        "main.generate_embeddings", return_value=mock_embeddings
    ) as mock_embed, patch(
        "main.write_products_to_alloydb", return_value=True
    ) as mock_write, patch(
        "main.vertexai.init"
    ) as mock_vertex_init:
        ...
        mock_vertex_init.assert_called_once_with(
            project="test-project", location="test-region"
        )


def test_run_worker_no_data():
    with patch.dict(os.environ, {"CLOUD_RUN_TASK_INDEX": "100"}), patch(
        "main.fetch_products", return_value=[]
    ) as mock_fetch, patch("main.vertexai.init"):
        success = run_worker()
        assert success is True
